...
**Legacy peer announcements.**
An `eth/72` node MUST NOT announce a type 3 transaction to a peer using an earlier `eth` protocol version unless it can serve the complete blob payload required by that version's `GetPooledTransactions` response. A node holding fewer than 64 distinct cells for any blob in the transaction MUST restrict the announcement to `eth/72` peers.
...
Consequently, samplers that cannot reconstruct the complete blob payload restrict their announcements to `eth/72` peers.
